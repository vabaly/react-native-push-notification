...
iOS 和 Android 通用的 React Native 本地和远程通知
...
**如果你是以 iOS 为目标就需要根据 [installation instructions for PushNotificationIOS](https://github.com/react-native-community/react-native-push-notification-ios) 来完成那些步骤**
...
**不要在组件内包括 `App` 根组件内中使用`.configure（）`。**
...
> 如果这样做，则通知处理程序将不会触发，因为它们未加载。 而是在应用程序的第一个文件（通常是index.js）中使用.configure（）。
...
Example文件夹包含一个示例应用程序，以演示如何使用此程序包。 通知处理在`NotifService.js`中完成。

请在提交示例之前使用此示例应用程序测试PR。 这将有助于维护此存储库。

## Handling Notifications

When any notification is opened or received the callback `onNotification` is called passing an object with the notification data.

当打开或接收到任何通知时，调用回调“ onNotification”被称为传递带有通知数据的对象。
...
PushNotification.localNotification({
  ...
  // 有效的唯一32位整数，指定为字符串。 默认值：自动生成的唯一ID
  ...
  // 显示通知时播放声音。 值'default'播放默认声音。 可以将其设置为自定义声音，例如“ android.resource：//com.xyz/raw/my_sound”。 它将在“ res / raw”目录中查找“ my_sound”音频文件并播放。 默认值：“默认值”（播放默认声音）
  soundName: "default", // (optional) Sound to play when the notification is shown. Value of 'default' plays the default sound. It can be set to a custom sound such as 'android.resource://com.xyz/raw/my_sound'. It will look for the 'my_sound' audio file in 'res/raw' directory and play it. default: 'default' (default sound is played)
  // 数量
  number: 10, // (optional) Valid 32 bit integer specified as string. default: none (Cannot be zero)
  // 重复间隔。 查看“重复通知”部分以了解更多信息。
